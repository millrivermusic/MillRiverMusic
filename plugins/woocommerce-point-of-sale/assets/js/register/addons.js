        ...
        processPayment: function (cart, payment_method) {
            switch (payment_method) {
                ...
                case 'paypal_pro':
                    ...
                    break;
                case 'paypal_pro_payflow':
                    var mounth = (jQuery('#cc-expire-month').val() < 10) ? '0' + jQuery('#cc-expire-month').val() : jQuery('#cc-expire-month').val();
                    var data = {
                        'paypal_pro_payflow-card-number': jQuery('#paypal_pro_payflow-card-number').val(),
                        'paypal_credit_card_rest-card-expiry': mounth + jQuery('#cc-expire-year').val(),
